...
{
    public Transform m_StartNode;   //  开始节点
    public Transform m_EndNode;     //  结束节点
    private GridBase m_Grid;              //  网格
    ...
    void Update()
    {
        FindingPath(m_StartNode.position, m_EndNode.position);      //  实时查找路径
    }
    ...
    private void FindingPath(Vector3 start, Vector3 end)                //  查找路径 [同时传入 起止节点]
    {
        Node startNode = m_Grid.GetFromPosition(start);                //  通过坐标获取起点的网格位置
        Node endNode = m_Grid.GetFromPosition(end);                  //  通过坐标获取终点的网格位置
        openList.Clear();                                                                   //  清空 list
        closeSet.Clear();                                                                   //  清空 list
        openList.Add(startNode);                                                     //  将起点 加入 openlist
